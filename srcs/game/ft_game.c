...
/*   Updated: 2023/01/23 13:37:32 by wmessmer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_actualisation(t_game *game)
{
	...
	mlx_string_put(game->mlx, game->win, (game->win_width / 2) - 48, \
		game->win_height + 12, 0xFF99FF, str);
	free(str);
	free(mov);
}
...
void	exit_animation(t_game *game)
{
	if (game->exit_loop == 0)
	{
		mlx_put_image_to_window(game->mlx, game->win, game->img_exit_open2, \
			game->exit_x * IMG_SIZE, game->exit_y * IMG_SIZE);
		game->exit_loop++;
	}
	else if (game->exit_loop == 1)
	{
		game->exit_loop = 0;
		mlx_put_image_to_window(game->mlx, game->win, \
		game->img_exit_open, game->exit_x * IMG_SIZE, game->exit_y * IMG_SIZE);
	}
}
